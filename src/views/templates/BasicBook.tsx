...
export const BasicBook: React.FC<BasicBookProps> = ({
    userId, onCreate, onExtractedTextChange, children
}) => {
    ...
    const hanleCreate = async() => {
        try{
          ...
          if(userId && selectedImage?.file){
            if(selectedImage)
              await onCreate(title, uploadHistory?.s3Path, description, uploadHistory?.extractedText);
            else{
    ...
    return (
        <Container>
            ...
            />
            {/* TODO: 이미지 업로드 버튼을 클릭했는지에 따라 업로드 버튼을 변경해야 함 */}
            {/* <Button onClick={hanleCreate}>완료</Button> */}
            {isClickUpload && <Button onClick={hanleCreate}>숨김버튼</Button>}
        </Container>
    );
}